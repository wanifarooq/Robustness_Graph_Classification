...
class FixedRepeatTransform(torch.nn.Module):
    def __init__(self, input_dim = 21, output_dim = 300):
        super(FixedRepeatTransform, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim

    def forward(self, x):
        # Repeat the input features to match the output dimension
        repeats = self.output_dim // self.input_dim
        remainder = self.output_dim % self.input_dim

        x_repeated = x.repeat(1, repeats)
        if remainder > 0:
            x_repeated = torch.cat([x_repeated, x[:, :remainder]], dim=1)
...
#         weight1 = self.linear.weight.data
#         # weight1 = (weight1+ weight1.T)/2
...
#         self.linear.weight.data = weight1
#         x = self.linear(x)
