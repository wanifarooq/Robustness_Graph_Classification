...
#     changeAbleIndexs, noise = select_random_values(class_index, args.percent, dataset.num_classes, index)
#     if 'ogbg' == args.dataset.split('-')[0]:
...
if 'ogbg' == args.dataset.split('-')[0]:
    if args.gnn == 'gin_ogbg':
        ...
        model_sp = GNN(gnn_type = 'gin', num_class = dataset.num_classes, num_layer = args.num_layer, emb_dim = args.emb_dim, drop_ratio = args.dropout_ratio, virtual_node = False).to(device)
    ...
    if args.gnn == 'dccnn':
        ...
        model_sp = GINClassifier(args.num_features, args.nhid, args.num_classes, args.num_layer).to(device)
    elif args.gnn == 'gcn_1':
...
def selectTheMode(x, x_prev, alpha):
    delta_x = abs(x - x_prev)+0.0001
    p =torch.tensor( (1 / (alpha * delta_x)))
    p = torch.clamp(p, min=0.01, max=1.0).item()
    # Now, sample 1 or 0 based on probability p
    result = np.random.choice([0, 1], p=[1-p, p])
    return result
...
#  these are for visualization only
    ...
    # if selectTheMode((correct_train / total_train),train_acc_cater,0.4)==1:
    if epoch%40==0:
