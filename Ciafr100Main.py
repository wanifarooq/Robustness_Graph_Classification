...
from datasethelpers.customdataset import CnnCifar, CnnCifar10, CnnMnist, get_random_ids
from loss import ncodLoss
from models.models_layer import CifarCnn, SimpleCNN, StrongCifarCnn
...
parser.add_argument('--gnn',              type=str,   default='CifarCnn', help='the type of the network needed , (default is GCN)')
...
parser.add_argument('--dataset',          type=str,   default="Cifar100",   help='the learning of the extra parameter')
...
dataset = CnnCifar10(root='data/Cifar10', train=True, download=True, transform=transform)
t_dataset = CnnCifar10(root='data/Cifar10', train=False, download=True, transform=transform)
dataset.targets=torch.Tensor(dataset.targets)
t_dataset.targets=torch.Tensor(t_dataset.targets)

# test_dataset = torchvision.datasets.MNIST(root='./data', train=False, download=True, transform=transform)
train_size = int(0.90 * len(dataset))
leave_size = len(dataset) - train_size
test_size = len(t_dataset) 
train_dataset,val_dataset = torch.utils.data.random_split(dataset, [train_size, leave_size])
test_dataset = torch.utils.data.Subset(t_dataset,[i for i in range(0,test_size)])
...
model = StrongCifarCnn(args.num_classes).to(device)
