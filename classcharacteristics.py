...
parser.add_argument('--gnn',              type=str,   default='gin_ogbg', help='the type of the network needed , (default is GCN)')
...
for index, indexes in enumerate(classbins, 0):
    # get the indices of each class of actual dataset
    class_index = train_dataset.dataset.train_indices[indexes]
    q=dataset[class_index]
    print(index,int(q[0].num_nodes/len(class_index)))
    dictclass[index] = int(q[0].num_nodes/len(class_index))
    dictnumber[index] = len(class_index)
changed_indices = []
allchangedindices = []
for index, indexes in enumerate(classbins, 0):
    # get the indices of each class of actual dataset
