...
from MLP_modules import SimpleClassifier
from customdataset import CustomizeData, CustomizeDataMnist, get_random_ids, select_random_values, Ogbdataset
import copy
import argparse

from dirchilet import dirichlet_energy
from gnn import GNN
from loss import ncodLoss
from sopLoss import overparametrization_loss
from Visulaize import visualization, makeImagedir, plot_grad_flow, plot_grad_u
from denserCluster import distribution, makedistdir
from models_layer import GINClassifier, DCCNN, HGPSLModel, GCNGraphClassifier, GRAFFGraphClassifier, \
...
#this thing is added for SOP only and needed to remove after experiment:
# soploss = overparametrization_loss(device,num_examp=len(train_dataset.indices),num_classes=args.num_classes)
# if soploss.USE_CUDA:
#     soploss.to(device)
# reparam_params = [{'params': soploss.u, 'lr': args.lr_u}, {'params': soploss.v, 'lr': 10}] 
# optimizer_sop = optim.SGD(reparam_params)
...
#put it equal to zero after test
train_acc_cater = 1.0
# Training loop

#remove after the test of non noisy:
# allnoisy = [id for sublist in noisyIndices for id in sublist]
...
def train(epoch):
    ...
    for i, data in  enumerate(tqdm(train_loader,  unit='batch')):
        ...
        #this is for sop loss
        # optimizer_sop.zero_grad()


        outputs, emb,node_embd = model(inputs,False)
        ...
        if ((epoch < args.intialbost) or (args.loss == 1)):
            ...
            #remove after the test is done for only pure training:, remember four lines to comment 
            #the criterion_reductionLess, the allNoisy and given two lines .
            # Effective_weightage = [0 if item in allnoisy else 1 for item in index_run]
            # loss  = (torch.tensor(Effective_weightage).to(device)*criterion_reductionLess(outputs,target)).mean()
            
            #this line is for sop loss again
            # loss = soploss(index_run,outputs,target)
        ...
        #this line is for the Sop again:
        # optimizer_sop.step()
    ...
    #uncomment after test this line
    ...
    if args.percent > 0:
        ...
        for classNumber, l in zip(range(4), otherClassIndices[:4]):
