...
from customdataset import CustomizeData, get_random_ids, select_random_values, Ogbdataset
import copy
import argparse
from f2gnn.f2gnnModel import NetworkGNN
...
parser.add_argument('--gnn',              type=str,   default='f2gnn', help='the type of the network needed , (default is GCN)')
...
parser.add_argument('--lr',               type=float, default=0.0005,     help='the learning rate of the network')
...
parser.add_argument('--lastlayerdim',     type=int,   default=512,        help='the dimesions of encoder layer, for ogbg dataset change it to 300,for others 64')
...
parser.add_argument('--arch_filename', type=str, default='f2gnn/arch.txt', help='given the location of searched res')
...
parser.add_argument('--batch_size', type=int, default=24, help='batch size of data.')
...
if 'ogbg' == args.dataset.split('-')[0]:
    if args.gnn == 'gin_ogbg':
        ...
        model= NetworkGNN(args.arch, nn.CrossEntropyLoss(), args.hidden_size, args.num_classes, args.hidden_size,
                    num_layers=args.num_layer, in_dropout=args.dropout,
                    out_dropout=args.dropout,
                    act=args.activation, args=args, is_mlp=args.is_mlp).to(device)
    else:
