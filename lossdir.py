...
class ncodLoss(nn.Module):
    ...
    def forward(self, index, outputs, label, out, flag, epoch,train_acc_cater,graph_sum):
        ...
        graph_sum = torch.stack(graph_sum).view(-1,1)
        graph_sum = graph_sum/max(graph_sum.detach())
        loss_dirch=torch.mean(u.detach()*graph_sum)
